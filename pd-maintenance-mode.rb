...
class PDMaintenance
  ...
  def self.pagerduty
    ...
    # Show info about PD schedule
      ...
      description = $params.description_given ? $params.description : "Automated maintenance mode."
      @options[:body] = {
        "maintenance_window" => {
          "service_ids"   => $params.project,
          "start_time"      => Time.now,
  ...
  def self.parse
    opts = Trollop::options do
      opt :project, "Pick specific PagerDuty project.", :type => :string
      opt :time, "Enable maintenance window for specific time (in minutes).", :type => :string
      opt :list_projects, "Shows list of the projects together with their ID"
